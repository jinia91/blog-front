import { type ExecuteState, type ICommand, type TextAreaTextApi } from '@uiw/react-md-editor'
import React, { Fragment, useCallback, useEffect, useRef, useState } from 'react'
import { getCodeString } from 'rehype-rewrite'
import mermaid from 'mermaid'

export const timestamp: ICommand = {
  name: 'timestamp',
  keyCommand: 'referenceLink',
  shortcuts: 'ctrlcmd+t',
  buttonProps: { 'aria-label': 'Add timeStamp (ctrl + t)', title: 'Add timeStamp (ctrl + t)' },
  icon: (
  ...
  execute: async (state: ExecuteState, api: TextAreaTextApi) => {
    const finalText = `<!-- timestamp: ${new Date().toISOString()} -->\n`
...
export const Code = ({ inline, children = [], className, ...props }: any): React.JSX.Element => {
  ...
  const language = (className !== undefined) ? className.split('-')[1] !== 'highlight' ? '' : className.split('-')[1] : ''
